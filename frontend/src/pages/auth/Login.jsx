...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(import.meta.env.VITE_BACKEND_URI + '/api/auth/login', { email, password });
      ...
      <div className="w-full max-w-md p-6 bg-[#812b71] text-[#f4ebf2] rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            ...
            <input
              type="email"
              className="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#d79ccc] bg-[#0d060c] text-[#f4ebf2] pt-5"
              value={email}
          ...
          <p className="text-center text-sm mt-4 text-[#f4ebf2]">Don't have an account? <Link to="/signup" className="text-[#d79ccc] hover:underline">Sign Up</Link></p>
        </form>
      </div>
    </div>
  );
